...
| --- | --- | --- |
| LangGraph | `@ag-ui/langgraph` / `ag-ui-langgraph` | ✅ The Python fork accepts only `RunAgentInput.resume[]` and always emits `RunFinishedEvent.outcome = {type:"interrupt"}`. Deprecated Python kwargs `emit_interrupt_outcome` and `enable_legacy_on_interrupt_event` remain accepted for source compatibility but are ignored. Legacy `forwardedProps.command.resume` and `CustomEvent(name="on_interrupt")` paths do not execute. Subclass hooks remain available for custom HITL translation. |
