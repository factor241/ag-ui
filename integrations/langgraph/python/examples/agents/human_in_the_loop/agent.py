...
AG-UI interrupt protocol note:
  The @ag-ui/langgraph integration now supports the AG-UI standard interrupt
  protocol. Clients resume execution only through RunAgentInput.resume =
  [ResumeEntry(interruptId=..., status="resolved", payload=...)], with one
  entry for every open interrupt. LangGraph uses the interrupt-ID mapping in
  the native Command(resume=...) and interrupt() receives its matched payload.
"""
